	...
	while(newsockfd = accept(sockfd, (struct sockaddr *) &cli_addr, &clilen)) { //accepts multiple connections
		...
		switch (spawnpid) {
			...
			case 0: //we are in the child process
				...
				if(fileNum == 2) {//Checks that the right client is connecting
					error("ERROR otp_dec not allowed to connect to otp_enc_d");
					exit(1);
				}
				...
				for (i = 0; i < strlen(message); i++){
					if(message[i] == ' ') { //Converts spaces to @ for easy encription
						message[i] = '@';
					}
